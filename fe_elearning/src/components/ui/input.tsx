...
const Input = React.forwardRef<HTMLInputElement, React.ComponentProps<'input'>>(
  ({ className, type, ...props }, ref) => {
    return (
      <div
        className={cn(
          'flex flex-row rounded-md border focus-within:ring-1 focus-within::ring-ring disabled:cursor-not-allowed disabled:opacity-50 shadow-sm border-input ring-lightSilver',
          className
        )}
      >
        <input
