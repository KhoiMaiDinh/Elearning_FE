  ...
  const errors = errorCodes.map((error) => {
    switch (error) {
      ...
      case 'file-too-large': {
        ...
        return `Kích thước tối đa là ${maxMb}MB`;
      }
      case 'file-too-small': {
        const roundedMinSize = limits.minSize
          ? (limits.minSize / (1024 * 1024)).toFixed(2)
          : 'negative?';
        return `Kích thước tối thiểu là ${roundedMinSize}MB`;
      }
      case 'too-many-files':
        return `Tối đa ${limits.maxFiles} file`;
    }
  });
...
const DropzoneMessage = forwardRef<HTMLParagraphElement, DropzoneMessageProps>((props, ref) => {
  ...
  return (
    <p
      ...
      {...rest}
      className={cn(
        'mb-1 font-medium text-[9px] text-redPigment h-[9px] text-start w-full ml-1',
        rest.className
      )}
    >
      {body}
