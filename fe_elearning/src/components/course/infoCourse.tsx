...
const InfoCourse: React.FC<InfoCourseProps> = ({ lecture, course }) => {
  ...
  const totalLessons = course.sections?.reduce((total, section) => total + section.items.length, 0) || 0
  const totalDuration =
    course.sections?.reduce(
      (total, section) => total + section.items.reduce((sum, item) => sum + (item.video?.duration_in_seconds || 0), 0),
      0
    ) || 0
  ...
  return (
    <div className="flex flex-col w-full font-sans md:gap-8 gap-4">
        ...
        <div className="flex flex-col justify-start gap-4 font-sans w-full md:w-2/3">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
            ...
            >
              <Heart
                size={16}
                className={cn("transition-all", isFavorite ? "fill-white text-white" : "text-current")}
              />
          ...
          <div className="flex flex-wrap items-center gap-3 font-sans">
            ...
            <div className="flex items-center font-sans gap-1 px-3 py-1.5 bg-purple-50 dark:bg-black50 rounded-full border border-white">
              ...
              <span className="text-xs font-medium text-purple-500">{formatDuration(totalDuration)}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2">
            <div className="flex flex-col">
              ...
              <span className="text-base font-medium text-black dark:text-white font-sans">{lecture}</span>
            </div>
            <div className="flex flex-col">
      ...
      <div className="flex flex-col font-sans bg-gray-50 dark:bg-gray-800/30 p-4 rounded-lg">
        <p className="font-medium text-gray-700 dark:text-gray-300 md:text-base text-sm font-sans">{course?.subtitle}</p>
      </div>

      {/* Course Description */}
      <div className="flex flex-col font-sans gap-3">
        <h3 className="font-bold text-lg md:text-xl text-eerieBlack dark:text-AntiFlashWhite font-sans">Mô tả khóa học</h3>
        <div
      ...
      <div className="flex flex-col font-sans gap-3 mt-2">
        <h3 className="font-bold text-lg md:text-xl text-eerieBlack dark:text-AntiFlashWhite font-sans  ">Nội dung khóa học</h3>
      ...
      {showReviews && (
        <Popup onClose={() => setShowReviews(false)}>
          <h3 className="text-lg font-semibold mb-4 font-sans">Tất cả đánh giá ({reviews.length})</h3>
